...
export default function CustomCakesPage() {
  ...
  // Define option structures with descriptions, prices, and icons/emojis
  const OPTIONS_1 = isBurger ? [
  ...
  // Sync state defaults when activeSite changes
  useEffect(() => {
    setOption1(OPTIONS_1[0].name);
    setOption2(OPTIONS_2[0].name);
    setOption3(OPTIONS_3[0].name);
  }, [activeSite]);
  ...
  return (
    <div className="min-h-screen pb-24 overflow-hidden relative z-10 bg-cream-bg text-charcoal transition-colors duration-500">
      ...
      <div className="absolute right-[-10%] top-[5%] w-96 h-96 opacity-[0.03] pointer-events-none float-animation">
        <i className={`fas text-[14rem] ${isBurger ? 'fa-hamburger' : 'fa-birthday-cake'}`} style={{ color: ACCENT_COLOR }}></i>
      </div>
      <div className="absolute left-[-5%] top-[40%] w-72 h-72 opacity-[0.02] pointer-events-none float-animation" style={{ animationDelay: '-3s' }}>
        <i className={`fas text-[10rem] ${isBurger ? 'fa-fire' : 'fa-seedling'}`} style={{ color: BRAND_COLOR }}></i>
      </div>
      ...
      <section className="relative pt-16 pb-12 md:pt-28 md:pb-16 px-4 text-center">
        <div className="max-w-4xl mx-auto">
          ...
          >
            {isBurger ? 'Hot, Cheesy, Tailored' : 'Dream It, We Bake It'}
          </span>
          ...
          >
            ...
            <span>{isBurger ? 'Consult Catering Studio' : 'Consult Design Studio'}</span>
      ...
      <section className="max-w-5xl mx-auto px-4 py-10 relative z-20">
        <div className="rounded-[2.5rem] p-6 sm:p-10 md:p-14 shadow-xl border relative overflow-hidden bg-cream-light border-accent-gold/25 transition-colors duration-500">
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
            ...
            <div className="lg:col-span-7 space-y-6">
              <div>
                ...
                <h2 className="text-2xl sm:text-3xl font-black font-serif-heading text-charcoal">
                  {isBurger ? 'Configure Your Party Combo' : 'Select Your Base Ideas'}
                </h2>
              ...
              <div className="space-y-3">
                <label className="block text-[10px] font-extrabold uppercase tracking-widest text-charcoal/40">
                  {isBurger ? '1. Base Fast Food Platter' : '1. Tier Structure & Shape'}
                </label>
              ...
              <div className="space-y-3">
                <label className="block text-[10px] font-extrabold uppercase tracking-widest text-charcoal/40">
                  {isBurger ? '2. Marine Drink Option' : '2. Design Theme / Style'}
                </label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {OPTIONS_2.map((opt) => (
                    <button
              ...
              <div className="space-y-3">
                <label className="block text-[10px] font-extrabold uppercase tracking-widest text-charcoal/40">
                  {isBurger ? '3. Hot Sides & Add-ons' : '3. Flavor Palette'}
                </label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {OPTIONS_3.map((opt) => (
                    <button
            ...
            {/* Right: Simulated Paper Grill Ticket / Receipt Plaque */}
              ...
              <div className="bg-white text-charcoal rounded-3xl p-6 sm:p-8 border border-neutral-300 shadow-xl flex flex-col justify-between flex-grow relative overflow-hidden font-mono min-h-[360px]">
                ...
                <div className="pt-2">
                  <div className="text-center border-b border-dashed border-neutral-400 pb-4 mb-4">
                    <h3 className="font-black uppercase tracking-wider text-sm">{brandName}</h3>
                    <p className="text-[8px] font-bold tracking-widest uppercase mt-0.5">JALANDHAR KITCHEN</p>
                    <p className="text-[7px] font-semibold opacity-60 mt-1">ORDER ID: #GF-{isBurger ? 'BURGER' : 'BAKER'}-{currentYear}</p>
                  </div>
                  ...
                  <div className="space-y-4 text-xs font-semibold">
                    ...
                    <div className="flex justify-between items-start gap-3">
                      <span>{isBurger ? 'DRINK' : 'THEME'}: {option2}</span>
                    ...
                    <div className="flex justify-between items-start gap-3 border-b border-dashed border-neutral-300 pb-4">
                      <span>{isBurger ? 'ADDON' : 'FLAVOR'}: {option3}</span>
                ...
                <div className="mt-8 border-t border-dashed border-neutral-400 pt-4">
                  ...
                  >
                    ...
                    <span>{isBurger ? 'Send Catering Receipt' : 'Send Ticket to Chef'}</span>
                  </button>
      ...
      <section className="max-w-6xl mx-auto px-4 py-12 relative z-10">
        <div className="text-center mb-10">
          ...
          <h3 className="text-2xl sm:text-3xl font-black text-charcoal font-serif-heading">
            {isBurger ? 'The Catering Process' : 'The Consultation Journey'}
          </h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2">
            <div className="p-8 sm:p-12 md:p-16 flex flex-col justify-center">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-accent-gold mb-3">
                {isBurger ? 'Catering Capabilities' : 'Customization Limits'}
              </span>
              ...
              <ul className="space-y-4 mb-10">
                {(isBurger 
                  ? ['Hot-box delivery for up to 100 guests', 'Live Counter setup available', 'Custom mocktail bar setup options', 'Tailored spice profiles per plate']
                  : ['3D Character & Toy Cakes', 'Gilded Multi-tier Wedding Cakes', 'Photo & Edible Sheet Print Cakes', 'Luxury Fresh Floral Cake Artistry']
                ).map((item, i) => (
            ...
            >
              <i className={`text-[12rem] text-white/[0.02] absolute animate-pulse fas ${isBurger ? 'fa-pizza-slice' : 'fa-hat-chef'}`}></i>
              <div className="relative z-10 text-center p-8">
                 <div className="bg-cream-light/10 backdrop-blur-md border border-white/10 p-8 rounded-2xl max-w-sm">
                    ...
                    <p className="text-xs uppercase tracking-wider mt-5 font-extrabold" style={{ color: ACCENT_COLOR }}>
                      {isBurger ? '- House Party Host, Jalandhar' : '- Birthday Celebration Customer'}
                    </p>
                 </div>
              </div>
            </div>
      ...
      <section className="max-w-3xl mx-auto px-4 py-8 relative z-10 text-center">
        ...
        <div className="grid grid-cols-1 gap-4">
          <div className="p-4 sm:p-5 bg-brand-plum/5 rounded-2xl text-brand-plum text-xs sm:text-sm font-semibold border border-accent-gold/20 flex items-center justify-center gap-3">
            ...
            <span>
              {isBurger ? (
                <>Please book larger catering orders at least <strong className="font-extrabold text-brand-plum">12 to 24 hours</strong> in advance to ensure fresh prep.</>
              ) : (
                <>Order at least <strong className="font-extrabold text-brand-plum">24 to 48 hours</strong> in advance for standard custom designs.</>
              )}
            </span>
          </div>
          <div 
