...
export default function HeroSection({ 
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const tiltX = -(y / (box.height / 2)) * 18; // Max 18 degrees tilt
  ...
  // Scroll listener for parallax ambient elements
  useEffect(() => {
    const handleScroll = () => {
  ...
  // Compute suggestions (min 2 chars)
  ...
  // Conditional elements based on activeSite
  ...
  return (
    <section className="relative z-40 w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12 pt-10 sm:pt-14 md:pt-20 pb-16 md:pb-24 overflow-hidden">
      ...
      <div className={`grid grid-cols-1 gap-8 items-center ${isBurger ? 'lg:grid-cols-12' : 'max-w-4xl mx-auto text-center'}`}>
        
        {/* Left/Main Column: Text & Search Content */}
        <div 
          className={`flex flex-col relative z-10 transition-transform duration-100 ease-out ${
            isBurger ? 'lg:col-span-7 text-left items-start' : 'items-center text-center'
          }`}
          style={{ transform: `translateY(${scrollY * 0.04}px)` }}
        >
          {/* Top Brand Tag */}
          <div className={`flex items-center gap-3 mb-6 ${isBurger ? 'justify-start' : 'justify-center'}`}>
          ...
          <span 
            ...
            style={{ 
              backgroundColor: `${activeSite === 'baker' ? '#7A1A40' : '#E63946'}0A`, 
              color: BRAND_COLOR, 
              borderColor: `${GOLD_COLOR}35` 
            }}
          >
            <i className="fas fa-star mr-2 text-xs" style={{ color: GOLD_COLOR }}></i> 
            {isBurger ? '100% Vegetarian Fast Food Kitchen' : 'Freshly Baked Everyday'}
          </span>
          
          <h1 className={`text-4xl sm:text-5xl md:text-6.5xl lg:text-7.5xl font-black mb-5 tracking-tight leading-[1.05] md:leading-[1.1] text-charcoal font-serif-heading ${
            isBurger ? 'text-left' : 'text-center'
          }`}>
            {isBurger ? (
              <>
                Spicy, Cheesy, <br />
          ...
          <p className={`text-charcoal/60 text-sm sm:text-base md:text-lg max-w-xl font-semibold leading-relaxed mb-8 md:mb-10 ${
            isBurger ? 'text-left' : 'text-center mx-auto'
          }`}>
            {isBurger ? (
          ...
          <div ref={searchRef} className="w-full relative max-w-2xl px-2 sm:px-0">
            ...
            >
              ...
              <div className="flex-1 w-full flex items-center relative h-full">
                ...
                <input 
                  type="text"
                  placeholder={isBurger ? "Search burgers, pizzas, sides..." : "Search wedding cakes, treats..."}
                  value={searchQuery}
        ...
        {/* Right Column: Burger Bro Parallax Presentation (Burger Mode only) */}
        {isBurger && (
          <div className="lg:col-span-5 relative flex justify-center items-center z-0 pt-10 lg:pt-0 min-h-[320px] sm:min-h-[460px]">
            ...
            >
              ...
              {/* Inner depth layer 3: Pop-out Ingredients (Lettuce, Chili, Cheese, Tomato) */}
              {/* Floating Chili */}
              <div 
