...
export default function BurgerProductModal({ 
  item, 
  isOpen, 
  onClose, 
  whatsappNumber, 
  ...
  const handleWhatsAppOrder = () => {
    if (isCustomSelected && !customWeight.trim()) {
      alert("Please enter your custom choice details before ordering.");
      return;
    }
  ...
  const handleShare = async () => {
    const pageUrl = typeof window !== 'undefined' ? window.location.href : '';
    const shareData = { title: `The Burger Bro - ${item.name}`, text: `Check out this tasty ${item.name}!`, url: pageUrl };
    if (navigator.share) {
        ...
        <div className="w-full md:w-1/2 p-6 sm:p-10 flex flex-col overflow-y-auto bg-cream-light relative">
          ...
          <div className="flex justify-between items-start gap-4 mb-4 border-b border-accent-gold/15 pb-5 shrink-0">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button 
                onClick={handleShare} 
                className="w-10 h-10 rounded-full flex items-center justify-center transition-colors shadow-sm bg-brand-primary/5 text-brand-primary hover:bg-brand-primary/10 cursor-pointer border border-accent-gold/20"
              >
          ...
          {/* Pricing details */}
          <div className="text-3xl font-black mb-4 shrink-0 text-brand-primary font-sans" style={{ color: brandColor }}>
            {displayPrice}
          </div>
          ...
          {hasVariants && (
            <div className="mb-8 p-5 bg-cream-bg rounded-2xl border border-accent-gold/20 shrink-0">
              <label className="block text-[10px] font-extrabold text-charcoal/40 uppercase tracking-widest mb-3">Select Variant / Option</label>
                ...
                {item.variantOptions?.map((opt: { label: string; price: string }, i: number) => (
                  <button
                    ...
                    style={{ 
                      ...
                      boxShadow: activeVarIdx === i ? `0 0 0 2px rgba(217, 179, 130, 0.1)` : 'none'
                    }}
                  >
                    {opt.label}
                  </button>
              ...
              {isCustomSelected && (
                <div className="mt-4 animate-in slide-in-from-top-2 duration-300">
                  <input
                    type="text"
                    placeholder="Enter custom request details..."
                    value={customWeight}
                    onChange={(e) => setCustomWeight(e.target.value)}
                    className="w-full text-xs sm:text-sm px-4 py-3.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-primary/20 bg-cream-light text-charcoal font-semibold border-accent-gold/30 transition-all shadow-sm"
                  />
                </div>
              )}
            </div>
